...
function SegmentRow({ seg, idx, total, today, onChange, onRemove, minDate }) {
  ...
  const dateFmt = seg.date ? fmtDateGoibibo(seg.date) : null

  return (
    <div className="flex flex-wrap md:flex-nowrap items-stretch border border-gray-200 rounded-xl overflow-visible mb-3 bg-white">
      {/* Segment label */}
      <div className="flex items-center justify-center w-full md:w-12 shrink-0 bg-orange-50 border-b md:border-b-0 md:border-r border-gray-200 px-2 py-2">
        <span className="text-xs font-bold text-orange-600">S{idx + 1}</span>
      </div>

      {/* FROM */}
      <div className="flex-1 min-w-0 px-4 pt-3 pb-3 border-b md:border-b-0 md:border-r border-gray-200 hover:bg-gray-50 transition-colors relative">
        <p className="text-xs text-gray-500 mb-1">From</p>
        <AirportSearch
          large
          value={seg.origin}
          onChange={v => onChange(idx, 'origin', v)}
      ...
      <div className="flex-1 min-w-0 px-4 pt-3 pb-3 border-b md:border-b-0 md:border-r border-gray-200 hover:bg-gray-50 transition-colors relative">
        ...
        <AirportSearch
          ...
          onChange={v => onChange(idx, 'destination', v)}
      ...
      {/* DATE */}
      <div
        className="w-full md:w-44 shrink-0 px-4 pt-3 pb-3 hover:bg-gray-50 transition-colors cursor-pointer select-none relative"
        onClick={openDate}
      >
        <p className="text-xs text-gray-500 mb-1 pointer-events-none">Date</p>
        {dateFmt ? (
          <>
            <p className="text-xl font-bold text-gray-900 leading-tight pointer-events-none">{dateFmt.primary}</p>
            <p className="text-xs text-gray-500 mt-0.5 pointer-events-none">{dateFmt.secondary}</p>
          </>
        ) : (
          <p className="text-sm text-blue-500 font-medium mt-1 pointer-events-none">Pick a date</p>
        )}
        <input
          ref={dateRef}
          type="date"
      ...
      {total > 2 && (
        <button
          ...
          className="flex items-center justify-center w-full md:w-10 shrink-0 border-t md:border-t-0 md:border-l border-gray-200 text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors text-lg font-bold py-2 md:py-0"
...
export default function FlightSearch({ initialValues = {}, onSearch }) {
  ...
  const [segments, setSegments] = useState(
    initialValues.segments || [
      { origin: '', destination: '', date: today },
      { origin: '', destination: '', date: today },
    ]
  )
  const [mcErrors, setMcErrors] = useState([])
  ...
  function openDeparture() {
    try { departureDateRef.current?.showPicker() } catch { departureDateRef.current?.focus() }
  }

  function openReturn() {
    if (tripType === 'one-way') setTripType('round-trip')
    setTimeout(() => {
      try { returnDateRef.current?.showPicker() } catch { returnDateRef.current?.focus() }
    }, 0)
  }
  ...
  function addSegment() {
    ...
    setSegments(prev => [...prev, { origin: last.destination || '', destination: '', date: last.date || today }])
  }
  ...
  function validateMultiCity() {
    ...
    for (let i = 0; i < segments.length; i++) {
      const s = segments[i]
      if (!s.origin)      { errors.push(`Segment ${i + 1}: origin is required`);      continue }
      if (!s.destination) { errors.push(`Segment ${i + 1}: destination is required`); continue }
      if (!s.date)        { errors.push(`Segment ${i + 1}: date is required`);        continue }
      if (s.origin.trim().toLowerCase() === s.destination.trim().toLowerCase())
        errors.push(`Segment ${i + 1}: origin and destination cannot be the same`)
      if (i > 0 && segments[i].date < segments[i - 1].date)
        errors.push(`Segment ${i + 1}: date must be on or after segment ${i} date`)
    }
    return errors
  }

  function handleMultiCitySearch(e) {
    e?.preventDefault()
    const errors = validateMultiCity()
    if (errors.length > 0) { setMcErrors(errors); return }
    const encoded = encodeURIComponent(JSON.stringify(segments))
    const params  = new URLSearchParams({ segments: encoded, passengers: adults, cabin, specialFare }).toString()
    if (onSearch) onSearch()
    navigate(`${ROUTES.MULTI_CITY_RESULTS}?${params}`)
  }
  ...
  // ── Shared: Travellers & Class panel ──
  function TravellersPanel() {
    return (
      <div className="relative cursor-pointer" onClick={() => setShowPax(p => !p)}>
        <div className="border border-gray-300 rounded-lg px-4 pt-3 pb-3 hover:bg-gray-50 transition-colors">
        ...
        <div>
          ...
          <div className="mb-3">
            {segments.map((seg, idx) => (
              <SegmentRow
                ...
                today={today}
                minDate={idx > 0 ? segments[idx - 1].date || today : today}
          ...
          {segments.length < 5 && (
            <button type="button" onClick={addSegment}
              className="flex items-center gap-2 text-sm font-semibold text-blue-600 hover:text-blue-800 mb-4 border border-dashed border-blue-300 hover:border-blue-500 rounded-lg px-4 py-2 transition-colors bg-blue-50 hover:bg-blue-100">
              <span className="text-lg leading-none">＋</span> Add City
            </button>
          )}

          {/* Travellers + cabin row */}
          <div className="mb-4">
            <TravellersPanel />
          </div>
          ...
          <div className="flex justify-center">
            <button type="button" onClick={handleMultiCitySearch}
              className="bg-orange-500 hover:bg-orange-600 text-white font-bold uppercase tracking-widest text-base px-20 py-4 rounded-full shadow-lg transition-colors">
              Search
            </button>
          </div>
        </div>
